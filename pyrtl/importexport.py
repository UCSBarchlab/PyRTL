...
class _VerilogOutput:
    ...
    def _to_verilog_header(self, file: TextIO, initialize_registers: bool):
    ...
    def _to_verilog_combinational(self, file: TextIO):
    ...
    def _to_verilog_sequential(self, file: TextIO):
    ...
    def _to_verilog_memories(self, file: TextIO):
    ...
    def _to_verilog_footer(self, file: TextIO):
        print("endmodule", file=file)

    def output_to_verilog(self, dest_file: TextIO, initialize_registers: bool):
...
def output_to_verilog(
    ...
    The Verilog module will have a clock input named ``clk``. If ``add_reset`` is not
    ``False``, there will also be a reset input named ``rst``.
    ...
    :param add_reset: If reset logic should be added. Allowable options are: ``False``
        ...
        reset to their :attr:`~.Register.reset_value`.
    :param initialize_registers: Initialize Verilog registers to their
    ...
    :param block: Block to be exported. Defaults to the :ref:`working_block`.
...
def output_verilog_testbench(
    ...
    :class:`.Input` values from a :class:`.SimulationTrace`.

    If ``add_reset`` is ``True``, ``output_verilog_testbench`` assumes the Verilog
    module under test has an input named ``rst``. But the testbench actually holds
    ``rst`` low, because all initialization is done in an ``initial`` block.

    ``output_verilog_testbench`` *only* generates the Verilog testbench. The Verilog
    module under test must be generated separately, by calling
    :func:`output_to_verilog`:

    1. Use :func:`output_to_verilog` to write the Verilog module under test to a file,
       like ``my_module.v``

    2. Use ``output_verilog_testbench`` to write the testbench to a different file, like
       ``my_module_testbench.v``
    ...
    This separates the hardware under test from its simulation test script, which makes
    it easier to simulate the same hardware with multiple test scripts, and to run the
    hardware through synthesis tools.

    The following example assumes :func:`output_to_verilog`'s output will be written to
    a file named ``my_module.v``::
    ...
    The Verilog testbench can be run with `Verilator <https://verilator.org/>`_,
    assuming it was written to ``my_module_testbench.v``:
    ...
    :param dest_file: An open file to which the test bench will be printed. Defaults to
        stdout.
    :param simulation_trace: A :class:`.SimulationTrace` from which :class:`.Input`
        values will be extracted for inclusion in the test bench. This parameter is
        typically set to :attr:`Simulation.tracer`. The generated test bench will replay
        the :class:`Simulation`'s :class:`Inputs<Input>` cycle by cycle. The default
        values for all registers and memories will be based on the
        :class:`.SimulationTrace`, otherwise they will be initialized to 0.
    :param toplevel_include: Name of the file containing the Verilog module generated by
        :func:`output_to_verilog`. If not ``None``, a Verilog ``include`` directive will
        be added for ``toplevel_include``.
    :param vcd: By default, the testbench generator will generate a command to write the
        ...
        be used.
    :param cmd: ``cmd`` will be copied verbatim into the testbench just before the end
        of each cycle. This is useful for printing specific values during testbench
    ...
    :param add_reset: If reset logic should be added. Allowable options are: ``False``
        (meaning no reset logic is added), ``True`` (default, for adding synchronous
        reset logic), and ``'asynchronous'`` (for adding asynchronous reset logic). Must
        match :func:`output_to_verilog`'s ``add_reset`` parameter.
    :param block: Block containing design to test. Defaults to the :ref:`working_block`.
    :param module_name: Name of the Verilog module. Must match
        :func:`output_to_verilog`'s ``module_name`` parameter.
    """  # noqa: E501
...
def output_to_firrtl(
    open_file: TextIO | None = None,
    rom_blocks: list[RomBlock] | None = None,
    ...
    :param open_file: File to write to. Defaults to stdout.
    ...
    print("circuit Example :", file=open_file)
    print("  module Example :", file=open_file)
    print("    input clock : Clock\n    input reset : UInt<1>", file=open_file)
    # write out IO signals, wires and registers
    for wire in _name_sorted(block.wirevector_subset(Input)):
        print(f"    input {wire.name} : UInt<{wire.bitwidth}>", file=open_file)
    for wire in _name_sorted(block.wirevector_subset(Output)):
        print(f"    output {wire.name} : UInt<{wire.bitwidth}>", file=open_file)
    for wire in _name_sorted(
        block.wirevector_subset(exclude=(Input, Output, Register, Const))
    ):
        print(f"    wire {wire.name} : UInt<{wire.bitwidth}>", file=open_file)
    for wire in _name_sorted(block.wirevector_subset(Register)):
        print(f"    reg {wire.name} : UInt<{wire.bitwidth}>, clock", file=open_file)
    for wire in _name_sorted(block.wirevector_subset(Const)):
        ...
        print(
            f"    node {wire.name} = UInt<{wire.bitwidth}>({wire.val})", file=open_file
        )
    ...
    for log_net in _net_sorted(block.logic_subset()):
        if log_net.op == "&":
            ...
            print(f"    {log_net.dests[0].name} <= not(T_{node_cntr})", file=open_file)
            node_cntr += 1
        elif log_net.op == "~":
            print(
                f"    {log_net.dests[0].name} <= not({log_net.args[0].name})",
                file=open_file,
            )
        elif log_net.op == "+":
            print(
            ...
            print(
                f"    {log_net.dests[0].name} <= {log_net.args[0].name}", file=open_file
            )
        elif log_net.op == "x":
            print(
            ...
            if rom_blocks is not None:
                ...
                print(f"    {log_net.dests[0].name} <= T_{node_cntr}", file=open_file)
                node_cntr += 1
        elif log_net.op == "@":
            if log_net.op_param[0] not in initializedMem:
                initializedMem.append(log_net.op_param[0])
            ...
            print(f"    when {log_net.args[2].name} :", file=open_file)
            print(
                f"      infer mport T_{node_cntr}  = {log_net.op_param[1].name}_"
                f"{log_net.op_param[0]}[{log_net.args[0].name}], clock",
                file=open_file,
            )
            print(f"      T_{node_cntr} <= {log_net.args[1].name}", file=open_file)
            print("      skip", file=open_file)
            node_cntr += 1
