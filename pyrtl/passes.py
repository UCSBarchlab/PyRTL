...
def timing_max_freq(timing_map=None, gate_delay_funcs=None, ffoverhead=189+194, block=None):
    """ Estimates the max frequency of a block in MHz.

    :param timing_map: timing_map to use (instead of generated one)
    :param gate_delay_funcs: dictionary of functions to pass to timing_analysis
    ...
    all params are optional and have resonable default values.
    """
    if not timing_map:
        timing_map = timing_analysis(block=block, gate_delay_funcs=None)
    cplength = timing_max_length(timing_map)
    clock_period_in_ps = cplength + ffoverhead
