...
def compile_c_to_library(cfilename, libfilename):
    """Compile a C program to a shared library / dynamically linked library.

    For use with the C programs generated by OutputToC.
    """
    ...
    subprocess.check_call([
        'gcc', '-xc', '-O0', '-march=native', '-std=c99', '-m64',
        shared, '-fPIC', '-o', libfilename, cfilename
        ], shell=(platform.system() == 'Windows'))  # TODO: is shell really needed here?


class OutputToC(object):
    """Create a C program for simulating the given block.
    ...
    and `n * self.output_size` for `outputs`.
    The offset for a given wire named `w` is `self.input_pos[w][0]` or `self.output_pos[w][0]`.
    ...
    Where <type> is the smallest of uint8_t, uint16_t, and uint32_t that will fit a single entry
    from the memory, or uint64_t if larger. The <varname> of the memory `m` is available with
    `self.varname[m]`. The <size> is the number of addresses in the memory (2**address width).
    If one entry of the memory is more than 64 bits wide, <limbs> is the number of uint64_t needed
    to store an entry; otherwise, it is 1. For example, a memory `m0_example` with bitwidth of 4
    and address width of 9 would be declared as
    ...
    def __init__(
        ...
        The destination file `dest_file` must be opened for writing.
        Default values for registers and memories are given by `register_value_map` and
        `memory_value_map`, respectively. Registers not present in the map are initialized to
        `default_value`, while memories not in the map are initialized to zero.
        """
        self._file = dest_file
    ...
    def _declare_mem(self, write, mem):
        ...
        if isinstance(mem, RomBlock):
            ...
            write('static const uint{width}_t {name}[][{limbs}] = {{'.format(
                name=vn, width=self._memwidth(mem), limbs=_limbs(mem)))
            for rv in romval:
            ...
            if mem in self._memmap:
                ...
                write('uint{width}_t {name}[{size}][{limbs}] = {{'.format(
                    ...
                    size=1 << mem.addrwidth, limbs=_limbs(mem)))
                for mv in memval:
    ...
    def _build_eq(self, write, op, param, args, dest):
        cond = []
        for n in range(max(_limbs(args[0]), _limbs(args[1]))):
            arg0 = self._getarglimb(args[0], n)
            arg1 = self._getarglimb(args[1], n)
    ...
    def _build_mul(self, write, op, param, args, dest):
        ...
        for p0 in range(_limbs(args[0])):
            write('carry = 0;')
            arg0 = self._getarglimb(args[0], p0)
            for p1 in range(_limbs(args[1])):
                if _limbs(dest) <= p0+p1:
                    break
            ...
            if _limbs(dest) > p0+_limbs(args[1]):
                write('{dest}[{p}] = carry{mask};'.format(
                    dest=self.varname[dest], p=p0+_limbs(args[1]),
                    mask=self._makemask(
                        dest, args[0].bitwidth+args[1].bitwidth, p0+_limbs(args[1]))))
    ...
    def _build_concat(self, write, op, param, args, dest):
        ...
        pieces = (
            ...
            for a in reversed(args) for l in range(_limbs(a)))
        curr = next(pieces)
        for n in range(_limbs(dest)):
            res = []
            dpos = 0
    ...
    def _create_code(self):
        ...
        self.input_pos = {}  # for each input wire, start and number of elements in input array
        self.input_bw = {}  # bitwidth of each input wire
        ipos = 0
        for w in inputs:
            self.input_pos[w.name] = ipos, _limbs(w)
            self.input_bw[w.name] = w.bitwidth
            for n in range(_limbs(w)):
                write('{vn}[{n}] = inputs[{pos}];'.format(vn=self.varname[w], n=n, pos=ipos))
                ipos += 1
        self.input_size = ipos  # total length of input array
        ...
        self.output_pos = {}  # for each output wire, start and number of elements in output array
        opos = 0
        for w in outputs:
            self.output_pos[w.name] = opos, _limbs(w)
            for n in range(_limbs(w)):
                write('outputs[{pos}] = {vn}[{n}];'.format(pos=opos, vn=self.varname[w], n=n))
                opos += 1
        self.output_size = opos  # total length of output array
        write('}')
...
class CompiledSimulation(object):
    ...
    def __init__(
        ...
        cfile = path.join(self._dir, 'pyrtlsim.c')
        libfile = path.join(self._dir, 'pyrtlsim.so')
