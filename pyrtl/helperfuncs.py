...
def probe(w: WireVector, name: str = None) -> WireVector:
    ...
    ``WireVector`` was originally created) and the run-time values of ``x`` (which will
    be named and thus show up by default in a trace). Likewise::
...
def rtl_assert(w: WireVector, exp: Exception, block: Block = None) -> Output:
    ...
    :return: The ``Output`` wire for the assertion, which can be ignored in most cases.
    """
...
def check_rtl_assertions(sim: Simulation):
    ...
    :class:`.Simulation` calls this automatically. See :func:`rtl_assert`.
...
def log2(integer_val: int) -> int:
    ...
    Useful when checking that powers of 2 are provided as function inputs. It raises an
    exception if a negative value is provided, or if the value provided is not an even
    power of two.
...
def truncate(wirevector_or_integer: Union[WireVector, int],
    ...
    result that is only ``bitwidth`` bits wide. For :class:`ints<int>` this is performed
    with a simple bitmask of size ``bitwidth``, and returns an :class:`int`. For
    ``WireVectors`` the function calls :meth:`.WireVector.truncate` and returns a
    ``WireVector`` with the specified ``bitwidth``.

    Examples::

        >>> truncate(0b101001, bitwidth=3)
        1
        >>> bin(truncate(0b101, bitwidth=3))
        '0b101'
        ...
        >>> x = WireVector(bitwidth=5)
        >>> y = truncate(x + 1, x.bitwidth)
        >>> y.bitwidth
        5

    :param wirevector_or_integer: A ``WireVector`` or :class:`int` to truncate.
    :param bitwidth: The length to which ``wirevector_or_integer`` should be truncated.

    :return: A truncated ``WireVector`` or :class:`int`, depending on the input type.
    """
    if bitwidth < 1:
...
def chop(w: WireVector, *segment_widths: int) -> list[WireVector]:
    ...
    This function chops a ``WireVector`` into a set of smaller ``WireVectors`` of
    different lengths. It is most useful when multiple "fields" are contained with a
    single ``WireVector``, for example when breaking apart an instruction. For example,
    if you wish to break apart a 32-bit MIPS I-type (Immediate) instruction you know it
    has an 6-bit opcode, 2 5-bit operands, and 16-bit offset. You could take each of
    those slices in absolute terms: ``offset=instr[0:16]``, ``rt=instr[16:21]`` and so
    on, but then you have to do the arithmetic yourself. With this function you can do
    all the fields at once which can be seen in the examples below.

    As a check, ``chop`` will throw an error if the sum of the lengths of the fields
    given is not the same as the length of the ``WireVector`` to ``chop``. Note also
    that ``chop`` assumes that the "rightmost" arguments are the least signficant bits
...
def wirevector_list(names: Union[str, list[str]],
    ...
    separated by a ``/``. This cannot be used in combination with a ``bitwidth`` value
    other than ``1``.
    ...
    .. WARNING::
        ...
        Avoid using this function. Create lists of ``WireVectors`` with list
        comprehensions, which are easier to understand because they compose familiar
        concepts, rather than introducing a new concept::
    ...
    :param names: Names for the ``WireVectors``. Can be a list or a single
        comma/space-separated string
    :param bitwidth: The desired bitwidth for the resulting ``WireVectors``.
    :param wvtype: The ``WireVector`` type to create.
    """
...
def val_to_signed_integer(value: int, bitwidth: int) -> int:
    ...
    Convert an unsigned integer (not a ``WireVector``!) to a signed integer. This is
    useful for printing and interpreting values which are negative numbers in two's
    complement::
...
def formatted_str_to_val(data: str, format: str, enum_set=None) -> int:
    ...
    Given a string (not a ``WireVector``!) convert that to an unsigned integer ready for
    input to the simulation environment. This helps deal with signed/unsigned numbers
    (simulation assumes the values have been converted via two's complement already),
    but it also takes hex, binary, and enum types as inputs. It is easiest to see how it
    ...
    :param format: A string holding a format which will be used to convert the data
        string.
    :param enum_set: An iterable of ``enums`` which are used as part of the conversion
        process.
...
def val_to_formatted_str(val: int, format: str, enum_set=None) -> str:
    ...
    Given an unsigned integer (not a ``WireVector``!) convert that to a human-readable
    string. This helps deal with signed/unsigned numbers (simulation operates on values
    that have been converted via two's complement), but it also generates hex, binary,
    and enum types as outputs. It is easiest to see how it works with some examples::
...
def infer_val_and_bitwidth(
    ...
    specified ``rawinput``. The tuple has fields ``.value`` and ``.bitwidth``. If
    ``signed`` is ``True``, bits will be included to ensure a proper two's complement
    representation is possible, otherwise it assumes a standard unsigned representation.
    Error checks are performed that determine if the bitwidths specified are sufficient
    and appropriate for the values specified. Examples::
...
def wire_struct(wire_struct_spec):
    ...
    ``WireVector`` subclass like :class:`Input` or :class:`Output` with
...
def one_hot_to_binary(w: WireVectorLike) -> WireVector:
    ...
    :param w: A ``WireVector``, or any type that can be coerced to ``WireVector`` by
        :func:`.as_wires`.
    :return: a ``WireVector`` containing the bit position of the high bit, in binary.
    """
...
def binary_to_one_hot(bit_position: WireVectorLike, max_bitwidth: int = None) -> WireVector:
    ...
    Examples::
        ...
        binary_to_one_hot(0)  # returns 0b01
        binary_to_one_hot(3)  # returns 0b1000
        binary_to_one_hot(0b100)  # returns 0b10000

    :param bit_position: A ``WireVector``, or any type that can be coerced to
        ``WireVector`` by :func:`.as_wires`.
    :param max_bitwidth: Optional integer maximum bitwidth for the resulting one-hot
        WireVector.

    :return: A ``WireVector`` with the bit at ``bit_position`` set to ``1`` and all
