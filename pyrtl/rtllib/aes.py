...
def g(word, key_expand_round):
    # STEP 1: One-byte left circular rotation.
    # STEP 2: Substitution of each byte of shifted word.
    a = [word[i:i+8] for i in range(0,32,8)]
    substituted_w3 = pyrtl.concat(sbox[a[2]], sbox[a[1]], sbox[a[0]], sbox[a[3]])
    # STEP 3: XOR substituted bytes with round constant.
    concat_w3 = pyrtl.concat( rcon[key_expand_round + 1], pyrtl.Const(0, bitwidth=24))
    return concat_w3 ^ substituted_w3


def key_expansion(in_vector):
    """ key expansion round of AES.

    Input: 16-byte key.
    Output: 176-byte expanded key.
    """
    w = []
    w.extend([in_vector[offset:offset+32] for offset in range(0,128,32)])
    for key_expand_round in range(10):
        last = key_expand_round * 4
        w.append(w[last] ^ g(w[last+3], key_expand_round))
