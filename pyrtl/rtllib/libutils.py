...
from pyrtl import helperfuncs, wire
...
def match_bitwidth(*args):
    ...
    """
    return helperfuncs.match_bitwidth(*args)


def basic_n_bit_mux(ctrl, mux_in, default=None):

    default = wire.Const(0) if default is None else default
    for ctrl_i in ctrl:
        next_mux_in = []
        for j in range((len(mux_in) + 1) // 2):
            second = default if 2*j + 1 >= len(mux_in) else mux_in[2*j + 1]
            next_mux_in.append(helperfuncs.mux(select=ctrl_i,
