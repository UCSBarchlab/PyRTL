...
def summation(matrix, axis=None, bits=None):
...
def minimum(matrix, axis=None, bits=None):
...
def maximum(matrix, axis=None, bits=None):
...
def argmax(matrix, axis=None, bits=None):
    ''' Returns the index of the maximum value of the matrix.
    ...
    NOTE: If there are two indices with the same maximum value, this function
    ...
    max_number = maximum(matrix, axis=axis, bits=bits)
    if axis is None:
        index = Const(0)
...
def dot(first, second):
    ...
    * Otherwise it is the matrix multiplaction between first and second
    ...
    if first.rows == 1:
        if second.rows == 1:
            return summation(first * second)
        if second.columns == 1:
            return summation(first * second.transpose())
