...
Debugging is half the coding process in software, and in PyRTL, it's no different. PyRTL
provides some additional challenges when it comes to debugging, as a problem may surface
long after the error was made. Fortunately, PyRTL comes with various features to help
you find mistakes.
"""
...
# sometimes it hard to track down where a wire might have come from, or what exactly it
# is doing.

# In this example specifically, we will be building a circuit that adds up three values.
# However, instead of building an add function ourselves or using the built-in "+"
# function in PyRTL, we will instead use the Kogge-Stone adders in RtlLib, the standard
# library for PyRTL.

# Building three inputs
in1, in2, in3 = (pyrtl.Input(8, f"in{x}") for x in range(1, 4))
...
# the simulation to trace the output. A simple "print" statement doesn't work because
# the values in the wires are not populated during *creation* time

# If we want to check the result of the first addition, we can connect an output wire to
# the result wire of the first adder
...
# always have the option to just pull the data out of the tracer directly
...
# way. We can start by clearing all of the hardware from the current working block. The
# working block is a global structure that keeps track of all the hardware you have
# built thus far. A "reset" will clear it so we can start fresh.
pyrtl.reset_working_block()
print("---- Using Probes ----")

# In this example, we will be multiplying two numbers using tree_multiplier() Again,
# create the two inputs and an output
in1, in2 = (pyrtl.Input(8, f"in{x}") for x in range(1, 3))
out1, out2 = (pyrtl.Output(8, f"out{x}") for x in range(1, 3))
...
# As one can see, probe can be used on any wire any time, such as before or during its
...
# to make any outputs! (although we did, to demonstrate the power and convenience of
# probes)
sim.tracer.render_trace()
sim.tracer.print_trace()

print("--- Probe w/ debugging: ---")
# Say we wanted to have gotten more information about one of those probes above at
# declaration. We could have used pyrtl.set_debug_mode() before their creation, like so:
...
# guaranteed to be preserved when functions (eg. pyrtl.synthesize() ) are done over the
# block.

# However, if the stack trace does not give you enough information about the WireVector,
# you can also embed additional information into the wire itself Two ways of doing so is
# either through manipulating the name of the WireVector, or by adding your own custom
# metadata to the WireVector.

# So far, each input and output WireVector have been given their own names, but normal
# WireVectors can also be given names by supplying the name argument to the constructor
...
# removing the WireVector from the block to prevent problems with the rest of this
# example
...
# output_to_trivialgraph will render your hardware in a format that you can then open
# with the free software "yEd" (http://en.wikipedia.org/wiki/YEd). There are options
# under the "hierarchical" rendering to draw something that looks quite like a circuit.
