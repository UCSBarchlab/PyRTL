...
persistent over clock cycles. In previous examples, we have shown the Register
WireVector, which is great for storing a small amount of data for a single clock cycle.
However, PyRTL also has other ways to store data, namely memories and ROMs.
"""
...
# Memories are a way to store multiple sets of data for extended periods of time. Below
# we will make two instances of the same memory to test that the same thing happens to
# two different memories using the same inputs
...
# In order to make sure that the two memories take the same inputs, we will use same
# write data, write enable, and read addr values
wdata = pyrtl.Input(32, "wdata")
we = pyrtl.Input(1, "we")
raddr = pyrtl.Input(3, "raddr")

# We will be grabbing data from each of the two memory blocks so we need two different
...
# Ports
#
# The way of sending data to and from a memory block is through the use of a port. There
# are two types of ports: read ports and write ports. Each memory can have multiple read
# and write ports, but it doesn't make sense for one to have either 0 read ports or 0
# write ports. Below, we will make one read port for each of the two memories
...
# allows us to disable the write port as long as the value is zero, giving us complete
# control over whether to accept the data.
...
# Now we will finish up the circuit. We will increment count register on each write
...
# inputs. Write 1 through 8 into the eight registers, then read back out
simvals = {
...
# Now run the simulation like before. Note the adding of the memory value map.
print("---------memories----------")
...
# ROMs are another type of memory. Unlike normal memories, ROMs are read-only and
# therefore only have read ports. They are used to store predefined data.

# There are two different ways to define the data stored in the ROMs, either through
...
# Now we will make the ROM blocks. ROM blocks are similar to memory blocks, but because
...
# Because Output WireVectors cannot be used as the source for other nets, in order to
# use the ROM outputs in two different places, we must instead assign them to a
# temporary variable.
...
# One of the things that is useful to have is repeatability, However, we also don't want
# the hassle of typing out a set of values to test. One solution in this case is to seed
# random and then pulling out 'random' numbers from it.
print("---------roms----------")
print(pyrtl.working_block())
random.seed(4839483)
...
# examples to parse values. The two ways we are doing it below are both valid ways of
# making larger values
...
# Now run the simulation like before. Note that for ROMs, we do not supply a memory
# value map because ROMs are defined with the values predefined.
