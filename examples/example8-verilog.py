...
While there is much more about PyRTL design to discuss, at some point somebody might ask
you to do something with your code other than have it print pretty things out to the
terminal. We provide import from and export to Verilog of designs, export of waveforms
to VCD, and a set of transforms that make doing netlist-level transforms and analysis
directly in PyRTL easy.
"""
...
# subcomponents of PyRTL designs or to be subject to analysis written over the PyRTL
# core. One standard format supported by PyRTL is "blif" format:
# https://www.ece.cmu.edu/~ee760/760docs/blif.pdf

# Many tools supoprt outputting hardware designs to this format, including the free open
# source project "Yosys". Blif files can then be imported either as a string or directly
# from a file name by the function input_from_blif. Here is a simple example of a 1-bit
# full adder imported and then simulated from this blif format.
...
# We are only going to trace the input and output vectors for clarity. Now simulate the
# logic with some random inputs
...
# However, not only do we want to have a method to import from Verilog, we also want a
# way to export it back out to Verilog as well. To demonstrate PyRTL's ability to export
# in Verilog, we will create a sample 3-bit counter. However unlike the example in
# example2, we extend it to be synchronously resetting.
...
# counter + 1. Note that both "0" and "1" are bit extended to the proper length and here
# we are making use of that native add operation. Let's dump this bad boy out to a
# Verilog file and see what is looks like (here we are using StringIO just to print it
# to a string for demo purposes; most likely you will want to pass a normal open file).
...
# The class OutputVerilogTestbench grabs the inputs used in the simulation trace and
# sets them up in a standard verilog testbench.
