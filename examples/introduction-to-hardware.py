...
#                             bit. This says to perform the following code 'n' times,
#                             but the value 'n' is passed as an input and is not
#                             something that is evaluated in the hardware; it is
#                             evaluated when you run the PyRTL program which generates
#                             (or more specifically elaborates) the hardware. Thus the
#                             hardware we are building will have the value of 'n' built
#                             into the hardware and won't actually be a run-time
#                             parameter. Loops are really useful for building large
#                             repetitive hardware structures, but they CAN'T be used to
#                             represent hardware that should do a computation
#                             iteratively. Instead we are going to need to use some
#                             registers to build a state machine.
#        a, b = b, a + b   -- Let's break this apart. In the first cycle b is Const(1)
#                             and (a + b) builds an adder with a (Const(0)) and b
#                             (Const(1)) as inputs. Thus (b, a + b) in the first
#                             iteration is: ( Const(1), result_of_adding( Const(0),
#                             Const(1) ) At the end of the first iteration "a" and "b"
#                             refer to those two constant values. In each following
#                             iteration more adders are built and the names "a" and "b"
#                             are bound to larger and larger trees of adders but all the
#                             inputs are constants!
#    return a              -- The final thing that is returned then is the last output
#                             from this tree of adders which all have Consts as inputs.
#                             Thus this hardware is hard-wired to find only and exactly
#                             the value of Fibonacci of the value N specified at design
#                             time! Probably not what you are intending.
...
#            └───────────┘
#
# Note that in the picture the register "a" and "b" each have a WireVector which is the
# current value (shown flowing out of the bottom of the register) and an "input" which
# is giving the value that should be the value of the register in the following cycle
# (shown flowing into the top of the register) which are "b" and "a + b" respectively.
# When we say "return a" what we are returning is a reference to the register "a" in the
# picture above.
