...
class TestWireVectorList(unittest.TestCase):
    ...
    def test_wirevector_list_type(self):
        ...
        wirevectors = pyrtl.wirevector_list("one, two, three")
    ...
    def test_wirevector_list_bitwidth(self):
        ...
        wirevectors = pyrtl.wirevector_list("one, two, three", 8)
        self.assertListEqual([wire.bitwidth for wire in wirevectors], [8, 8, 8])

    def test_wirevector_list_per_wire_width(self):
        wirevectors = pyrtl.wirevector_list("one/2, two/4, three/8")
        self.assertListEqual([wire.bitwidth for wire in wirevectors], [2, 4, 8])

        wirevectors = pyrtl.wirevector_list(["one", "two", "three"], [2, 4, 8])
        self.assertListEqual([wire.bitwidth for wire in wirevectors], [2, 4, 8])

    def test_wirevector_list_raise_errors(self):
        with self.assertRaises(ValueError):
            pyrtl.wirevector_list(["one", "two", "three"], [2, 4])

        with self.assertRaises(pyrtl.PyrtlError):
            pyrtl.wirevector_list("one/2, two/4, three/8", 16)

        with self.assertRaises(pyrtl.PyrtlError):
            pyrtl.wirevector_list(["one/2", "two/4", "three/8"], [8, 4, 2])
...
class TestNonCoreHelpers(unittest.TestCase):
    ...
    def test_infer_val_and_bitwidth(self):
        ...
        self.assertEqual(
            pyrtl.infer_val_and_bitwidth(-3, bitwidth=5, signed=True), (29, 5)
        )
...
class TestMatchBitpattern(unittest.TestCase):
    ...
    def test_pattern_type_or_length_mismatch(self):
        ...
        with self.assertRaises(pyrtl.PyrtlError):
            pyrtl.match_bitpattern(instr, "000100010")
        with self.assertRaises(pyrtl.PyrtlError):
            pyrtl.match_bitpattern(instr, "0001000")
        with self.assertRaises(pyrtl.PyrtlError):
            pyrtl.match_bitpattern(instr, "0b00010001")
        with self.assertRaises(pyrtl.PyrtlError):
            pyrtl.match_bitpattern(instr, 0b000100010)
        with self.assertRaises(pyrtl.PyrtlError):
            pyrtl.match_bitpattern(instr, "")
        with self.assertRaises(pyrtl.PyrtlError):
            pyrtl.match_bitpattern(instr, None)
        with self.assertRaises(pyrtl.PyrtlError):
            pyrtl.match_bitpattern(instr, instr)
    ...
    def check_all_accesses_valid(self):
        ...
        output = io.StringIO()
        sim.tracer.print_trace(output, compact=True)
        self.assertEqual(output.getvalue(), "  i 26313\nout 313\n")
...
class TestBitpatternToVal(unittest.TestCase):
    ...
    @staticmethod
    def r5_br_immed(x):
        # i is the 12th bit, k is the 11th bit, and j is the bottom 10 bits of the
        # immediate field.
    ...
    def test_ordered_fields(self):
        self.assertEqual(
            pyrtl.bitpattern_to_val(
                "0000000sssssrrrrr000ddddd0110011", 1, 2, 3
            ),  # RISC-V ADD
        ...
        self.assertEqual(
            pyrtl.bitpattern_to_val(
                "iiiiiiisssssrrrrr010iiiii0100011", 1, 3, 4
            ),  # RISC-V SW
    ...
    def test_named_fields(self):
        ...
        self.assertEqual(
            pyrtl.bitpattern_to_val(
                "iiiiiiisssssrrrrr010iiiii0100011", i=1, s=3, r=4
            ),  # RISC-V SW
            0b00000000001100100010000010100011,
        )
        ...
        self.assertEqual(
            pyrtl.bitpattern_to_val(
                ...
                s=2,
                r=3,
                **TestBitpatternToVal.r5_br_immed(-5),
            ),  # RISC-V BLT
            0b11111110001000011100101111100011,
        )
    ...
    def test_named_fields_with_field_map(self):
        ...
        self.assertEqual(
            pyrtl.bitpattern_to_val(
                "0000000sssssrrrrr000ddddd0110011",
                rs2=1,
                rs1=2,
                rd=3,
                field_map=field_map,
    ...
    def test_no_fields(self):
        self.assertEqual(pyrtl.bitpattern_to_val("1010010"), 0b1010010)
...
class TestChop(unittest.TestCase):
    ...
    def test_fields_mismatch(self):
        ...
        with self.assertRaises(pyrtl.PyrtlError):
            pyrtl.chop(instr, 10, 10, 10)
        with self.assertRaises(pyrtl.PyrtlError):
            pyrtl.chop(instr, 10, 10, 14)
        with self.assertRaises(pyrtl.PyrtlError):
            pyrtl.chop(instr, 33)
    ...
    def test_wrong_input_types_fail(self):
        ...
        with self.assertRaises(pyrtl.PyrtlError):
            pyrtl.chop(instr, x, 10, 12)
        with self.assertRaises(pyrtl.PyrtlError):
            pyrtl.chop(instr, 10, x, 12)
        with self.assertRaises(pyrtl.PyrtlError):
            pyrtl.chop(instr, x)
        with self.assertRaises(pyrtl.PyrtlError):
            pyrtl.chop(10, 5, 5)
...
class TestBitField_Update(unittest.TestCase):
    ...
    def test_field_too_big(self):
        ...
        with self.assertRaises(pyrtl.PyrtlError):
            pyrtl.bitfield_update(a, 1, 2, b)

    def test_field_too_big_truncate(self):
        a = pyrtl.WireVector(name="a", bitwidth=3)
        b = pyrtl.WireVector(name="b", bitwidth=3)
        pyrtl.bitfield_update(a, 1, 2, b, truncating=True)

    def test_no_bits_to_update(self):
        a = pyrtl.WireVector(name="a", bitwidth=3)
        b = pyrtl.WireVector(name="b", bitwidth=3)
        with self.assertRaises(pyrtl.PyrtlError):
            pyrtl.bitfield_update(a, 1, 1, b, truncating=True)

    def bitfield_update_checker(
        self, input_width, range_start, range_end, update_width, test_amt=20
    ):
        def ref(i, s, e, u):
            mask = ((1 << (e)) - 1) - ((1 << s) - 1)
            return (i & ~mask) | ((u << s) & mask)

        inp, inp_vals = utils.an_input_and_vals(
            input_width, test_vals=test_amt, name="inp"
        )
        upd, upd_vals = utils.an_input_and_vals(
            update_width, test_vals=test_amt, name="upd"
        )
...
class TestBitField_Update_Set(unittest.TestCase):
    ...
    def test_field_too_big(self):
        ...
        b = pyrtl.WireVector(name="b", bitwidth=3)
        with self.assertRaises(pyrtl.PyrtlError):
            pyrtl.bitfield_update_set(a, {(1, 2): b})

    def test_field_too_big_truncate(self):
        a = pyrtl.WireVector(name="a", bitwidth=3)
        b = pyrtl.WireVector(name="b", bitwidth=3)
        pyrtl.bitfield_update_set(a, {(1, 2): b}, truncating=True)

    def test_no_bits_to_update(self):
        a = pyrtl.WireVector(name="a", bitwidth=3)
        b = pyrtl.WireVector(name="b", bitwidth=3)
        with self.assertRaises(pyrtl.PyrtlError):
            pyrtl.bitfield_update_set(a, {(1, 1): b}, truncating=True)
    ...
    def test_overlapping_ranges(self):
        ...
        with self.assertRaises(pyrtl.PyrtlError):
            pyrtl.bitfield_update_set(a, {(1, 4): b, (3, 6): c}, truncating=True)
        with self.assertRaises(pyrtl.PyrtlError):
            pyrtl.bitfield_update_set(a, {(8, 10): b, (-1, None): c}, truncating=True)
        with self.assertRaises(pyrtl.PyrtlError):
            pyrtl.bitfield_update_set(a, {(None, 3): b, (0, 1): c}, truncating=True)
    ...
    def bitfield_update_set_checker(
        ...
        update_set_list = []
        for ix, (range_start, range_end, update_width) in enumerate(
        ...
        for ix, iv in enumerate(inp_vals):
            true_result.append(
                reduce(
                    lambda w, ui: ref(
                        w, ui.range_start, ui.range_end, ui.update_vals[ix]
                    ),
                    update_set_list,
                    iv,
                )
...
class TestTreeReduce(unittest.TestCase):
    ...
    def test_xor(self):
        wires, vals = utils.make_inputs_and_values(
            7, exact_bitwidth=8, dist=utils.uniform_dist
        )
...
class TestMux(unittest.TestCase):
    ...
    def test_mux_too_many_inputs_with_default(self):
        ...
        s = pyrtl.WireVector(name="s", bitwidth=2)
        with self.assertRaises(pyrtl.PyrtlError):
            pyrtl.mux(s, a, b, c, d, e, default=0)
...
class TestMuxSimulation(unittest.TestCase):
    ...
    def mux_t_subprocess(self, addr_width, val_width):
        mux_ins, vals = utils.make_consts(
            num_wires=2**addr_width, exact_bitwidth=val_width
        )
    ...
    def test_mux_with_default(self):
        ...
        num_defaults = 5
        mux_ins, vals = utils.make_consts(
            num_wires=2**addr_width - num_defaults,
            exact_bitwidth=val_width,
            random_dist=utils.uniform_dist,
        )
        control, testctrl = utils.an_input_and_vals(
    ...
    def test_select(self):
        ...
        control, testctrl = utils.an_input_and_vals(
            1, 40, "sel_ctrl", utils.uniform_dist
        )
...
class TestShiftSimulation(unittest.TestCase):
    ...
    def shift_checker(
        ...
        if shift_amount is not None:
            ...
            shf, shf_vals = utils.an_input_and_vals(
                shift_width, test_vals=test_amt, name="shf"
            )
            inputs.append(shf)
    ...
    def sll_checker(self, input_width, shift_width, shift_amount=None):
        ...
        self.shift_checker(
            pyrtl.shift_left_logical, ref, input_width, shift_width, shift_amount
        )
    ...
    def sla_checker(self, input_width, shift_width, shift_amount=None):
        ...
        self.shift_checker(
            pyrtl.shift_left_arithmetic, ref, input_width, shift_width, shift_amount
        )
    ...
    def srl_checker(self, input_width, shift_width, shift_amount=None):
        ...
        self.shift_checker(
            pyrtl.shift_right_logical, ref, input_width, shift_width, shift_amount
        )
...
class TestWireStruct(unittest.TestCase):
    ...
    def test_exceptions(self):
        # Slicing exceptions.
        with self.assertRaises(pyrtl.PyrtlError):
            Byte(Byte=0xAB, high=0xC)
        with self.assertRaises(pyrtl.PyrtlError):
            Byte(Byte=0xAB, foo=0xC)
    ...
    def test_pixel_concatenate(self):
        Pixel(
            name="pixel", red=Byte(name="foo", high=0xA, low=0xB), green=0xCD, blue=0xEF
        )
...
class TestWireMatrix(unittest.TestCase):
    ...
    def test_wire_matrix_concatenate(self):
        BitPair(name="bitpair", values=[1, 0])
    ...
    def test_wire_matrix_word_concatenate(self):
        Word(name="word", values=[0xAB, 0xCD])
