...
class TestBarrel(unittest.TestCase):
    ...
    def test_shift_left(self):
        ...
        self.out_zeros <<= barrel_shifter(
            self.inp_val, bit_in=0, direction=Direction.LEFT,
            shift_dist=self.inp_shift)
        self.out_ones <<= barrel_shifter(
            self.inp_val, bit_in=1, direction=Direction.LEFT,
            shift_dist=self.inp_shift)
    ...
    def test_shift_right(self):
        ...
        self.out_zeros <<= barrel_shifter(
            self.inp_val, bit_in=0, direction=Direction.RIGHT,
            shift_dist=self.inp_shift)
        self.out_ones <<= barrel_shifter(
            self.inp_val, bit_in=1, direction=Direction.RIGHT,
            shift_dist=self.inp_shift)
