...
Logic is then created when wires are combined with one another using the provided operators.  For example, if `a` and `b` are both of type :class:`.WireVector`,
the `a + b` will make an adder, plug `a` and `b` into the inputs of that adder, and return a new :class:`.WireVector` which is the output of that adder.
...
The classes :class:`.Input`, :class:`.Output`, :class:`.Const`, and :class:`.Register` are all derived from :class:`.WireVector`, but
extend it with (or restrict it from) with certain functionality.  The :class:`.Input` and :class:`.Output`
...
design).  The :class:`.Const` class is useful for specifying hard-wired values, while :class:`.Register` is
